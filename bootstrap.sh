...
main() {
    ...
    setup_symlinks # needed if you're setting up vim and tmux setup_vim and setup_tmux
...
function install_vscode_extensions() {
    ...
    for extension_to_install in "${extensions[@]}"
    do
        if echo "$extensions_outcome" | \
            grep --ignore-case "$extension_to_install" &> /dev/null; then
            substep "\"${extension_to_install}\" already exists"
        else
            if code --install-extension "$extension_to_install"; then
                substep "Extension \"${extension_to_install}\" installation succeeded"
            else
    ...
    success "vscode extensions successfully installed"
}
...
function setup_symlinks() {
    ...
    success "Symlinks successfully setup, skipped hammerspoon karabiner tmux vim"
}
...
function setup_symlinks() {
    ...
    success "Symlinks successfully setup, skipped hammerspoon karabiner powerline tmux vim"
}
